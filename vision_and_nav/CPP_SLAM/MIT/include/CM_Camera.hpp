...
public:
  ...
  typeCameraPose(const Eigen::Matrix3d &RNew, const Eigen::Vector3d &tNew)
      : typePose{RNew, tNew}, Quaternion(RNew.transpose()),
  ...
  void UpdateRt(void) {
    const Eigen::Matrix3d RTransposed =
        Quaternion.toRotationMatrix().transpose();
...
typeCamera CM_PredictPose(const typeCameraPose &TPreviousFrame,
                          const typeCameraPose &TPreviousPreviousFrame);
