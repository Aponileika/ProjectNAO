...
#define PANTO_DBOW_BUILD_VOCAB false



namespace fs = std::filesystem;

constexpr std::string VocabFilePath("PantoVocabulary.dbow3");
...

{
    ...
    std::cout << "Saved vocab to " << VocabFilePath << "\n";
    ...
    DBoW3::Vocabulary SavedVocab(VocabFilePath);
    ...
    const fp64 SameDatasetFrameDistanceStdDev = std::sqrt(

            SameDatasetFrameDistanceVariance);



    const fp64 SelfTop1Rate = !TrainingDescriptors.empty()

        ? static_cast<fp64>(SelfTop1Count) /

            static_cast<fp64>(TrainingDescriptors.size())

        : 0.0;



    const fp64 BestNonSelfSameDatasetRate = ValidBestNonSelfCount > 0

        ? static_cast<fp64>(BestNonSelfSameDatasetCount) /

            static_cast<fp64>(ValidBestNonSelfCount)
