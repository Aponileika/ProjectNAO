...
{
    LG_Log(LogSeverity::DBG, "[AKAZE_InitAKAZE] initing akaze with %lf\n", threshold);
    ...
    akaze->matchratio = PANTO_MATCHRATIO;
    return akaze;
}
...
{
    LG_Log(LogSeverity::DBG, "[AKAZE_GetMatches] extr void* = %p\n", extractor);
    struct OpenCVExtractAKAZE* akaze = static_cast<struct OpenCVExtractAKAZE*>(extractor);
    cv::Mat des1, des2;

    LG_Log(LogSeverity::DBG, "[AKAZE_GetMatches] img1 empty=%d rows=%d cols=%d type=%d channels=%d data=%p\n",
       img1.empty(), img1.rows, img1.cols, img1.type(), img1.channels(), img1.data);

    LG_Log(LogSeverity::DBG, "[AKAZE_GetMatches] img2 empty=%d rows=%d cols=%d type=%d channels=%d data=%p\n",
       img2.empty(), img2.rows, img2.cols, img2.type(), img2.channels(), img2.data);
    LG_Log(LogSeverity::DBG, "[AKAZE_GetMatches] detect and compute img1\n");
    ...
    LG_Log(LogSeverity::DBG, "[AKAZE_GetMatches] detect and compute img2\n");
    ...
    LG_Log(LogSeverity::DBG, "[AKAZE_GetMatches] num corrp akaze = %d\n", out.first.size());
