...
void __VIZ_WriteImagesColmap(struct ObservationSet os, struct PointSet ps, struct ViewSet vs, FILE* fp);
...
void __VIZ_WriteImagesColmap(struct ObservationSet os, struct PointSet ps, struct ViewSet vs, FILE* fp)
{
    ...
    for(u64 i = 0; i < num_images; i++)
    {
        ...
        struct Param* params = vs.views[i].p;
        const fp64 qw = params->q.w();
        const fp64 qx = params->q.x();
        const fp64 qy = params->q.y();
        const fp64 qz = params->q.z();

        const fp64 quat[4] = {qw, qy, qz, qx};
