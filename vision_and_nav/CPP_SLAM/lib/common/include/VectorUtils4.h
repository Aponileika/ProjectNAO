...
//		double x, y, z, w; // w or h
		...
		{double z; double b;};
		union
		{double h; double w; double a;};
		#ifdef __cplusplus
            vec4() {}
			vec4(double x2, double y2, double z2, double w2) : x(x2), y(y2), z(z2), w(w2) {}
			vec4(double xyz, double w2) : x(xyz), y(xyz), z(xyz), w(w2) {}
			vec4(vec3 v, double w2) : x(v.x), y(v.y), z(v.z), w(w2) {}
...
//		double x, y;
		...
		#ifdef __cplusplus
            vec2() {}
			vec2(double x2, double y2) : x(x2), y(y2) {}
		#endif
	} vec2, *vec2Ptr;
	...
	typedef struct mat4
	{
		double m[16];
			...
			mat4(double p0, double p1, double p2, double p3,
	...
	typedef struct mat3
	{
		double m[9];
			...
			mat3(double p0, double p1, double p2,
...
// New better name for SetVector and replacements for constructors
	vec2 SetVec2(double x, double y);
	vec3 SetVec3(double x, double y, double z);
	vec4 SetVec4(double x, double y, double z, double w);

	vec3 SetVector(double x, double y, double z);
	mat3 SetMat3(double p0, double p1, double p2, double p3, double p4, double p5, double p6, double p7, double p8);
	mat4 SetMat4(double p0, double p1, double p2, double p3,
				double p4, double p5, double p6, double p7,
				double p8, double p9, double p10, double p11, 
				double p12, double p13, double p14, double p15
				);
...
// Note: C++ users can also use operator overloading, defined below.
	...
	vec3 ScalarMult(vec3 a, double s);
	double Norm(vec3 a);
...
// Mat3 operations (new)
	...
	mat4 ArbRotate(vec3 axis, double fi);
...
// GLU replacement functions
	...
	mat4 lookAt(double px, double py, double pz, 
			double lx, double ly, double lz,
			double vx, double vy, double vz);
	mat4 perspective(double fovyInDegrees, double aspectRatio,
                      double znear, double zfar);
	mat4 frustum(double left, double right, double bottom, double top,
                  double znear, double zfar);
	mat4 ortho(double left, double right, double bottom,
...
// NEW as prototype 2022, added to VU 2023
	void uploadMat4ToShader(unsigned int shader, const char *nameInShader, mat4 m);
	void uploadUniformIntToShader(unsigned int shader, const char *nameInShader, int i);
	void uploadUniformFloatToShader(unsigned int shader, const char *nameInShader, double f);
	void uploadUniformFloatArrayToShader(unsigned int shader, const char *nameInShader, double *f, int arrayLength);
	void uploadUniformVec3ToShader(unsigned int shader, const char *nameInShader, vec3 v);
	void uploadUniformVec3ArrayToShader(unsigned int shader, const char *nameInShader, vec3 *a, int arrayLength);
	void bindTextureToTextureUnit(unsigned int tex, int unit);
...
// --- vec2 operations ---
// Added by joner335 and jonya700, used for barycentric coordinates
...
double operator*(const vec2 &a, const vec2 &b) // vec2-vec2
{
...
inline
double operator*(const vec3 &a, const vec3 &b) // vec3 dot vec3
{
	return (a.x*b.x+ a.y*b.y+ a.z*b.z);
}

//added by joner335, element wise multiplication operator overload for %,
//the above * should probably be this but I stick by the choice made for simplicity
inline
vec3 operator%(const vec3 &a, const vec3 &b) // vec3 dot vec3
{
	return SetVector(a.x*b.x, a.y*b.y, a.z*b.z);
}
...
inline
void operator*=(vec3 &a, const double &b) // vec3*=scalar
{
	a = a * b;
}
...
vec4 operator-(const vec4 &a, const vec4 &b) // vec4-vec4
{
	...
	// Questionable, not like GLSL
inline
double operator*(const vec4 &a, const vec4 &b) // vec4 dot vec4
{
...
inline
void operator*=(vec4 &a, const double &b) // vec4 *= scalar
{
	a = a * b;
}
...
// Like above; Modern C doesn't need this, but Visual Studio insists on old-fashioned C and needs this.
	...
	double Norm(vec3 a)
	{
		...
		result = (double)sqrt(a.x * a.x + a.y * a.y + a.z * a.z);
		return result;
	}
	...
	vec3 normalize(vec3 a)
	{
		...
		norm = (double)sqrt(a.x * a.x + a.y * a.y + a.z * a.z);
		result.x = a.x / norm;
		result.y = a.y / norm;
		result.z = a.z / norm;
		return result;
